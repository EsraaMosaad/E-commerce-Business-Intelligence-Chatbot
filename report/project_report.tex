...
Group 25fltp}
\author{
    \textbf{Data Engineering} — EMR Preprocessing\\
    \textbf{Model Fine-Tuning} — QLoRA Fine-tuning\\
    \textbf{System Deployment} — EC2 + OpenWebUI
}
\date{\today}
...
\midrule
Data Engineering & EMR preprocessing, PySpark pipeline, S3 pipeline, data quality \\
Model Fine-tuning & Colab notebook, QLoRA fine-tuning, GGUF export, RAG backend \\
System Deployment & VPC, EC2, Ollama, OpenWebUI, Terraform, CI/CD \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item Grocery\_and\_Gourmet\_Food
    \item Pet\_Supplies
\end{itemize}
...
The PySpark pipeline processed 9 categories and extracted a total of \textbf{360,277} highly structured training pairs, alongside \textbf{44,874} validation samples and \textbf{44,849} test samples, yielding a total corpus of \textbf{450,000} documents optimized for fine-tuning.

\textbf{Category Distribution (Training):}
\begin{itemize}
    \item Books: 40,116
    \item Pet Supplies: 40,177
    \item Home \& Kitchen: 40,052
    \item Clothing, Shoes \& Jewelry: 40,077
    \item Beauty \& Personal Care: 40,008
    \item Toys \& Games: 39,997
    \item Sports \& Outdoors: 39,981
    \item Grocery \& Gourmet Food: 39,966
    \item Electronics: 39,903
\end{itemize}

\textbf{Sentiment Class Balance (Training):}
\begin{itemize}
    \item Positive: 294,776
    \item Negative: 38,252
    \item Neutral: 27,249
\end{itemize}

\textbf{Token Lengths:}
The generated executive summaries feature extensive detail. The training set has an average character length of \textbf{285.91} (Max: 31,709 characters). The validation and test sets feature similar depths with averages of \textbf{284.65} and \textbf{286.99} respectively, ensuring the fine-tuned model learns to generate in-depth, multi-paragraph strategic reports rather than static, short sentences.
...
Master Instance: m6i.2xlarge (8 vCPU, 32 GB RAM)
Core Instances: 2 × m6i.2xlarge
Bootstrap: s3://25fltp-ecom-chatbot/scripts/emr-bootstrap.sh
\end{verbatim}
